...
const prefix = `${API_SERVER_HOST}/api/gocamping`    //내것은 이것을 씁니다.

//번호얻는 함수getOne 
export const getOne3 = async (tno) => { //async 이거안에서만 await(기다려줘)라는 것을 쓸 수 있습니다.
    const res = await axios.get(`${prefix}/${tno}`) //그 번호를 조회해서 우리한테 데이터를 넘겨줘
    return res.data

}
//async 비동기 처리
export const getList3 = async(pageParam) => {
